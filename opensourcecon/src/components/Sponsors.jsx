    ...
    <section id="sponsors" className="py-20 lg:py-28 relative">
      <div className="max-w-container mx-auto px-6 lg:px-8">
        ...
        <div className="text-center mb-12">
          ...
          <div className="card max-w-sm mx-auto p-8 text-center hover:shadow-md transition-shadow">
            <div className="flex justify-center mb-4">
              {titleSponsor.logo ? (
                <img src={titleSponsor.logo} alt={titleSponsor.name} className="h-8 object-contain" />
              ) : null}
            </div>
            <h3 className="font-heading font-extrabold text-3xl mb-2">{titleSponsor.name}</h3>
            <p className="text-gray-secondary text-sm">{titleSponsor.tagline}</p>
          </div>
        </div>
        ...
        <div className="text-center mb-12">
          ...
          <div className="flex flex-wrap justify-center gap-4">
            {poweredBy.map((sponsor) => (
              <div
                key={sponsor.name}
                className="card py-4 px-6 min-w-[150px] flex items-center justify-center gap-3 hover:shadow-md transition-shadow"
              >
                {sponsor.logo ? (
                  <img src={sponsor.logo} alt={sponsor.name} className="h-5 object-contain" />
                ) : (
                  <div className="w-5 h-5 rounded bg-gray-200 flex-shrink-0" />
                )}
                <span className="font-heading font-bold text-sm">{sponsor.name}</span>
              </div>
            ))}
          </div>
        </div>
        ...
        {/* CTA Strip */}
        <div className="card p-6 md:p-8 flex flex-col md:flex-row items-center justify-between gap-6 bg-gradient-to-r from-gray-50 to-white">
          <div>
